...
export function Toolbar({
  ...
  return (
    <div className="space-y-2">
      <div
        className={`relative flex items-center gap-3 p-2 bg-slate-900/90 rounded-xl border border-cyan-500/20 backdrop-blur-md transition-all duration-200 ${className}`}
      >
        <AnimatePresence>
          {activeNotification && (
            <motion.div
            ...
            >
              <div className="rounded-full bg-cyan-500 px-3 py-1 text-slate-950 font-bold text-xs shadow-lg">
                {items.find((item) => item.id === activeNotification)?.title}{" "}
                clicked!
              </div>
              <motion.div
                animate="animate"
                className="absolute -bottom-1 left-1/2 h-[2px] w-full origin-left bg-cyan-500"
                exit="exit"
                initial="initial"
        ...
        <div className="flex items-center gap-2">
          {items.map((item) => (
            <motion.button
              animate="animate"
              className={`relative flex items-center rounded-lg px-3 py-2 font-medium text-sm transition-colors duration-300 ${
                selected === item.id
                  ? "bg-[#1F9CFE] text-white shadow-md shadow-cyan-500/30"
            ...
            >
              <item.icon
                className={selected === item.id ? "text-white" : "text-slate-400"}
                size={16}
              />
          ...
          <motion.button
            className={`flex items-center gap-2 px-4 py-2 rounded-xl border shadow-sm transition-all duration-200 ${
              isToggled
                ? "bg-[#1F9CFE] text-white border-[#1F9CFE]/30"
